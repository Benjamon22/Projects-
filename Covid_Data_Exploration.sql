...
with vac_percent as (
	SELECT DISTINCT(dea.date), dea.continent, dea.location, dea.population, vac.new_vaccinations, (SUM(vac.new_vaccinations) OVER (Partition by dea.location ORDER by dea.date))/2 as VaccinePerDay
	From Covid_deaths dea
		Join Covid_vac vac
		On dea.location = vac.location AND dea.date = vac.date 
        Order by dea.location, dea.date
)
SELECT *, (VaccinePerDay/population)*100 as VaccinePercentage
FROM vac_percent

-- Create View for Data Visualization--
Create View vac_percent as 
SELECT DISTINCT(dea.date), dea.continent, dea.location, dea.population, vac.new_vaccinations, (SUM(vac.new_vaccinations) OVER (Partition by dea.location ORDER by dea.date))/2 as VaccinePerDay
	From Covid_deaths dea
		Join Covid_vac vac
		On dea.location = vac.location AND dea.date = vac.date 
